...
// Queue Dependency
var FetchQueue		= require("./redis-queue.js"),
...
/*
	...
	initialPort			-	Port to begin crawl1ing from.
...
var Crawler = function(host,initialPath,initialPort,interval) {
	...
	crawler.retries = 1;
	crawler.retryWaitInterval = 10 * 1000;
...
Crawler.prototype.start = function() {
	...
	crawler.queue.getLength(function(err,queueLength) {
...
Crawler.prototype.queueLinkedItems = function(resourceData,queueItem,decompressed) {
	...
	// BN: We don't crawl the URLs collected from other websites
	if (URI(queueItem.url).domain() != crawler.host) resources = [];
	...
	if (crawler.domainValid(parsedURL.host)) {
		crawler.queue.add(
			...
			function queueAddCallback(error,newQueueItem) {
				if (error && error.code != "DUP" && error.code != 'ENOTFOUND') console.log(error);
				if (error) {
...
Crawler.prototype.fetchQueueItem = function(queueItem) {
	...
	process.nextTick(function() {
		...
		clientRequest.on("error",function(errorData) {
			if (errorData && errorData.code != 'ENOTFOUND') console.log(errorData);
			crawler._openRequests --;
			...
			if (queueItem.retries <= 0) {
				...
				crawler.queue.returnToQueue(queueItem, function(err,queueItem) {
					if (err && err.code != 'ENOTFOUND') console.log(err); });
				crawler.emit("retryfailed",queueItem,null,queueItem.retries);
			}			
		});
...
Crawler.prototype.handleResponse = function(queueItem,response,timeCommenced) {
	...
	// Ignore this request, but record that we had a 404
		...
		if (queueItem.retries <= 0) {
			...
			crawler.queue.returnToQueue(queueItem, function(err,queueItem) {
				if (err && err.code != 'ENOTFOUND') console.log(err); });
			crawler.emit("retryfailed",queueItem,response,queueItem.retries);
		}		
...
Crawler.prototype.crawl = function() {
	...
	console.log('OpenRequests:', crawler._openRequests);
	...
	crawler.queue.oldestUnfetchedItem(function(err,queueItem) {
		...
		if (queueItem) {
			...
			crawler.queue.getLength(function(err,queueLength) {
				if (err) console.log(err);
				if (queueLength === 0) {
					crawler.emit("complete", crawler.host);
					crawler.stop();
					return crawler;
				}
			});
