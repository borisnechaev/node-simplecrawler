...
FetchQueue.prototype.returnToQueue = function(queueItem,callback) {
	...
	client.hset('hashes', queueItem.url, JSON.stringify(queueItem), function(error) {
		if (error) { 
			callback(new Error("Error inserting into Redis: ", error));
		}});
	client.lpush('fifo', queueItem.url, function(error) {
		if (error) { 
			callback(new Error("Error inserting into Redis: ", error));
		}});
};
